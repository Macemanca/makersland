...
public class TerrainDebugWindow {
	...
	// debug menu
	private Label lblFPS, lblChunksHeight, lblChunksWidth, lblOctaveCount, lblOctaves, lblScale, lblSeaLevel,
			lblStrength;
	private Skin skin;
	private Slider sliderChunksHeight, sliderChunksWidth, sliderOctaveCount, sliderOctaves, sliderScale, sliderSeaLevel,
			sliderStrength;
	public Stage stage;
	...
	private void setupWindow() {
		...
		buttonGenerate.addListener(new ChangeListener() {
			...
			public void changed(ChangeEvent event, Actor actor) {
				...
				gameView.setTerrain(new Terrain((int) sliderOctaves.getValue(), (int) sliderOctaveCount.getValue(),
						sliderStrength.getValue(), sliderScale.getValue(), (int) sliderChunksWidth.getValue(),
						(int) sliderChunksHeight.getValue(), 0, cbIsland.isChecked(), seed));
